...
addEventListener("load", function() {
  var cpuURL;
  cpuURL = URL.createWorkerURL(`import '${location.href}/prototype.js'; import '${location.href}/0ptr_window.js'; addEventListener( 'message', function ({ data }){ self.memory = data.memory.defineProperties(); self.postMessage(0); memory.lock(1); console.error('cpu unlocked', name ); /* user code evaulating: */${script} });`);
  self.bridge = new Worker(URL.createWorkerURL(`import '${location.href}/prototype.js'; import '${location.href}/0ptr_window.js'; self.memory = new SharedArrayBuffer(); self.postMessage({ memory: self.memory, name }); memory.lock(); console.warn( 'bridge unlocked:', name ); /* user code evaulating: */${script}`));
