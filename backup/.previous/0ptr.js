...
self.dump = function() {
  var childs, dmp, dumps, index, ptri, ptrj;
  childs = [];
  index = 0;
  ptri = +this;
  ptrj = offset();
...
export var Pointer = (function() {
  class Pointer extends Number {
    ...
    subarray(offset = 0, byteLength, TypedArray) {
      ...
      byteLength || (size = getByteLength(this));
      length = byteLength / TypedArray.BYTES_PER_ELEMENT;
      return new TypedArray(sab, byteOffset, length);
    }

    dataView(offset = 0, byteLength, TypedArray) {
      byteOffset += getByteOffset(this);
      byteLength || (size = getByteLength(this));
...
export var NumberPointer = class NumberPointer extends Pointer {
  ...
  static from(value = 0, ProtoPtr = this) {
    ...
    if (ProtoPtr === NumberPointer) {
      ...
      ProtoPtr = !Number.isSafeInteger(number) ? Number.isInteger(number) ? "BigInt64" : Float32Number : number < 0 && (absval = number * -2) ? absval <= 0xff ? Int8Number : absval <= 0xffff ? Int16Number : absval <= 0xffffffff ? Int32Number : (number = BigInt(number)) ? "BigInt64" : void 0 : number <= 0xff ? Uint8Number : number <= 0xffff ? Uint16Number : number <= 0xffffffff ? Uint32Number : (number = BigInt(number)) ? "BigUint64" : void 0;
...
export var LinkedStringPointer = (function() {
  class LinkedStringPointer extends StringPointer {};

  Object.defineProperty(LinkedStringPointer.prototype, "target", {
    enumerable: true,
    get: function() {
      return Pointer.of(getScopeIndex(this));
    },
    set: function() {
      return setScopeIndex(arguments[0], this);
    }
  });

  return LinkedStringPointer;
...
export var ClassPointer = (function() {
  class ClassPointer extends ObjectPointer {
    ...
    getAllocLength(size = 0) {
      var j, len, o, parent, ref;
...
Object.defineProperty(LinkedStringPointer.prototype, "children", {
  enumerable: true,
  configurable: true,
  get: Pointer.prototype.filter
});
